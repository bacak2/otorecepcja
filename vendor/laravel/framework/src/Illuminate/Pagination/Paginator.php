...
class Paginator extends AbstractPaginator implements Arrayable, ArrayAccess, Countable, IteratorAggregate, JsonSerializable, Jsonable, PaginatorContract
{
    ...
    protected function setItems($items)
    {
        ...
        $lastKey = $this->items->keys()->last();

        $this->items = $this->items->slice($this->currentPage*$this->perPage-$this->perPage, $this->perPage);

        if($lastKey == $this->items->keys()->last()) $this->hasMore = false;
